...
> **BUILT LOCALLY – After cloning, navigate into the project directory and build.**

> **Fork of [octo-fiesta](https://github.com/V1ck3s/octo-fiesta)** (original by v1ck3s)  
> 
> **Changes in this fork:**
> - **MUSIC_SERVICE**: Added `monochrome` option – Tidal via instance list (with automatic failover)
> - **SquidWTF Tidal**: Uses SquidWTF's own Tidal API – supports MP3 and lossless
> - **Monochrome**: Dedicated option for Tidal via instance failover when SquidWTF is unavailable
> - Simplified docker-compose – all settings via `.env`
> - Anonymized `.env.example` with clearer placeholders
...
|----------|---------------------|-------------|------------------|
| [SquidWTF](https://squid.wtf/) | No | FLAC 24-bit (Qobuz), MP3 + Lossless (Tidal) | Yes (Tidal) |
| Monochrome | No | FLAC (Tidal) | Yes |
...
When `MUSIC_SERVICE=SquidWTF`, `SQUIDWTF_SOURCE` controls which backend is used:

| `SQUIDWTF_SOURCE` | Content | Failover |
|-------------------|---------|----------|
| `Qobuz` *(default)* | Qobuz | No |
| `Tidal` | Tidal (MP3 + lossless) | No |
...
**Use Tidal via SquidWTF (MP3 + lossless):**
```env
MUSIC_SERVICE=SquidWTF
...
**Use Tidal via Monochrome (instance failover):**
```env
MUSIC_SERVICE=monochrome
