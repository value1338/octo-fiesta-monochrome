...
public class SubsonicController : ControllerBase
{
    ...
    /// <summary>
    /// Legacy search endpoint (search2). Arpeggi und andere Clients nutzen search2 statt search3.
    /// Ohne diesen Handler werden Suchanfragen an Navidrome weitergeleitet → nur lokale Ergebnisse.
    /// </summary>
    [HttpGet, HttpPost]
    [Route("rest/search2")]
    ...
    public async Task<IActionResult> Search2()
    {
        ...
        if (string.IsNullOrWhiteSpace(cleanQuery))
        {
            try
            {
                var result = await _proxyService.RelayAsync("rest/search2", parameters);
                var contentType = result.ContentType ?? $"application/{format}";
                return File(result.Body, contentType);
            }
            catch
            {
                return _responseBuilder.CreateResponse(format, "searchResult2", new { });
            }
        }

        var subsonicTask = _proxyService.RelaySafeAsync("rest/search2", parameters);
        var externalTask = _metadataService.SearchAllAsync(
            cleanQuery,
            int.TryParse(parameters.GetValueOrDefault("songCount", "20"), out var sc) ? sc : 20,
            int.TryParse(parameters.GetValueOrDefault("albumCount", "20"), out var ac) ? ac : 20,
        ...
        Task<List<ExternalPlaylist>> playlistTask = _subsonicSettings.EnableExternalPlaylists
            ? _metadataService.SearchPlaylistsAsync(cleanQuery, ac)
            : Task.FromResult(new List<ExternalPlaylist>());
        ...
        return MergeSearchResults2(subsonicResult, externalResult, playlistResult, format);
    }
    ...
    public async Task<IActionResult> GetArtist()
    {
        ...
        if (externalArtists.Count > 0)
        {
            ...
            if (StringNormalizer.ArtistNamesMatch(externalArtist.Name, artistName))
            {
        ...
        _logger.LogDebug("getArtist merge for '{ArtistName}': {LocalCount} local + {ExternalCount} external = {TotalCount} albums",
            artistName, localAlbums.Count, externalAlbums.Count, mergedAlbums.Count);
    ...
    public async Task<IActionResult> GetAlbum()
    {
        ...
        if (PlaylistIdHelper.IsExternalPlaylist(id))
        {
            try
            {
                ...
                // Use track.Id (playback ID) not ext-{provider}-{externalId} - for Spotify playlists, tracks
                // are Tidal songs with Id="ext-squidwtf-song-{tidalId}", so the cache key must match.
                if (_playlistSyncService != null)
                {
                    foreach (var track in tracks)
                    {
                        ...
                        {
                            _playlistSyncService.AddTrackToPlaylistCache(track.Id, id);
                        }
                    }
    ...
    private IActionResult MergeSearchResults2(
        (byte[]? Body, string? ContentType, bool Success) subsonicResult,
        SearchResult externalResult,
        List<ExternalPlaylist> playlistResult,
        string format)
    {
        ...
        var isJson = format == "json" || subsonicResult.ContentType?.Contains("json") == true;
        var (mergedSongs, mergedAlbums, mergedArtists) = _modelMapper.MergeSearchResults(
            localSongs, localAlbums, localArtists, externalResult, playlistResult, isJson);
        ...
        if (isJson)
        {
            return _responseBuilder.CreateJsonResponse(new
            {
                status = "ok",
                version = "1.16.1",
                searchResult2 = new { song = mergedSongs, album = mergedAlbums, artist = mergedArtists }
            });
        }

        var ns = XNamespace.Get("http://subsonic.org/restapi");
        var searchResult2 = new XElement(ns + "searchResult2");
        foreach (var artist in mergedArtists.Cast<XElement>()) searchResult2.Add(artist);
        foreach (var album in mergedAlbums.Cast<XElement>()) searchResult2.Add(album);
        foreach (var song in mergedSongs.Cast<XElement>()) searchResult2.Add(song);
