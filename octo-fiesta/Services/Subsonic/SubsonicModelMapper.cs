...
public class SubsonicModelMapper
{
        ...
        try
        {
            ...
            if (contentType?.Contains("json") == true)
            {
                ...
                if (jsonDoc.RootElement.TryGetProperty("subsonic-response", out var response) &&
                    (response.TryGetProperty("searchResult3", out searchResult) || response.TryGetProperty("searchResult2", out searchResult)))
                {
    ...
    private (List<object> MergedSongs, List<object> MergedAlbums, List<object> MergedArtists) MergeSearchResultsJson(
        ...
        List<ExternalPlaylist> externalPlaylists)
    {
        // Externe zuerst – damit Top Hits / erste Treffer in Arpeggi & Co. API-Ergebnisse zeigen
        var mergedSongs = externalResult.Songs.Select(s => _responseBuilder.ConvertSongToJson(s))
            .Concat(localSongs)
            .ToList();
        
        var mergedAlbums = externalResult.Albums.Select(a => _responseBuilder.ConvertAlbumToJson(a))
            .Concat(externalPlaylists.Select(p => ConvertPlaylistToAlbumJson(p)))
            .Concat(localAlbums)
            .ToList();
        
        var externalArtistNames = new HashSet<string>(externalResult.Artists.Select(a => a.Name ?? ""), StringComparer.OrdinalIgnoreCase);
        var mergedArtists = externalResult.Artists.Select(a => _responseBuilder.ConvertArtistToJson(a)).ToList();
        foreach (var localArtist in localArtists)
        {
            if (localArtist is Dictionary<string, object> dict && dict.TryGetValue("name", out var nameObj))
            {
                var name = nameObj?.ToString() ?? "";
                if (!externalArtistNames.Contains(name))
                    mergedArtists.Add(localArtist);
            }
            else
            {
                mergedArtists.Add(localArtist);
        ...
        List<ExternalPlaylist> externalPlaylists)
    {
        ...
        foreach (var artist in localArtists.Cast<XElement>())
        {
            ...
            if (string.IsNullOrEmpty(name) || !externalArtistNames.Contains(name))
            {
